...
class NN(object):
    def __init__(self, input_number, hidden_number, output1_number, output2_number):
    ...
    def extract_weights(self,chromosome):
        ...
        W2_Rot_end=W1_end+(self.output1_number*self.hidden_number)
        W2_X_end=W2_Rot_end+(self.output2_number*self.hidden_number)

        W1=chromosome[:W1_end]
        W2_Rot=chromosome[W1_end:W2_Rot_end]
        W2_X=chromosome[W2_Rot_end:W2_X_end]

        return W1.reshape(self.hidden_number,self.input_number), W2_Rot.reshape(self.output1_number, self.hidden_number), \
    ...
    def forward(self,chromosome, input):
        W1, W2_rot, W2_X=self.extract_weights(chromosome)

        Z=np.matmul(W1,input.T)
        A=self.sigmoid(Z)
        Rot_Z=np.matmul(W2_rot, A)
