...
if __name__ == "__main__":
    ...
    size_w = input_number * hidden_number + hidden1_number*hidden_number+ hidden_number* output1_number + hidden_number * output2_number
    size = (pop_size, size_w)
    population = np.random.choice(np.arange(-1, 1, step=0.01), size=size, replace=True)

    network=NN(input_number, hidden_number,hidden1_number,output1_number, output2_number)
